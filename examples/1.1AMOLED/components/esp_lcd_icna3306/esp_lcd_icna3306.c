...
static const char *TAG = "icna3306";

esp_err_t esp_lcd_new_panel_icna3306(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config,
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ESP_RETURN_ON_FALSE(panel_dev_config && ret_panel, ESP_ERR_INVALID_ARG, TAG, "Invalid arguments");
    icna3306_vendor_config_t *vendor_config = NULL;
    if (panel_dev_config->vendor_config) {
        vendor_config = (icna3306_vendor_config_t *)panel_dev_config->vendor_config;
    }
    ...
    if (vendor_config && vendor_config->flags.use_mipi_interface) {
        ret = esp_lcd_new_panel_icna3306_mipi(io, panel_dev_config, ret_panel);
    ...
    ret = esp_lcd_new_panel_icna3306_spi(io, panel_dev_config, ret_panel);
